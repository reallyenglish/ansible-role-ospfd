...
describe file(config) do
  ...
  its(:content) { should match(/^no redistribute #{Regexp.escape("127.0.0.0/8")}$/) }
...
describe command("ospfctl show int") do
  its(:stdout) { should match(/^em0\s+#{Regexp.escape("10.0.2.15/24")}\s+DOWN\s+-\s+active\s+00:00:00\s+0\s+0$/) }
end
